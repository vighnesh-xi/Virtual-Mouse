...
capture = cv2.VideoCapture(0)           # ✅ Fixed: 0 for built-in webcam
...
pyautogui.FAILSAFE = False              # ✅ Prevents crash when cursor hits screen edge
pyautogui.PAUSE = 0                     # ✅ Removes default 0.1s delay on every action
    ...
    if len(landmarksList) != 0:
        ...
        if fingersState[1] == 1 and fingersState[2] == 1:
            ...
            if distance < 40:
                ...
                pyautogui.click()
                time.sleep(0.3)         # ✅ Debounce — prevents multi-click on one gesture
